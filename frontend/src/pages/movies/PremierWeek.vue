...
<template>
    <BaseLayout>
        <div v-if="!loading" class="min-h-screen text-white flex flex-col items-center py-12 px-4 space-y-20">
            <div class="min-h-screen flex items-center -mt-6 lg:-mt-12 align-middle w-full mx-auto">
            ...
            <div v-for="film in films" :key="film.id"
                ...
                <div class="md:w-1/2 flex flex-col justify-center gap-4">
                    <h2 class="text-2xl md:text-3xl font-bold text-amber-400 tracking-wide">{{ film.movie.title }}
                    </h2>
                    <div>
                        <p class="mb-2 text-pink-50 text-sm md:text-base font-semibold">{{ film.movie.description }}</p>
                    </div>
                    <div class="grid grid-cols-2 gap-y-4 border-y-2 border-y-slate-600/50 py-4 w-11/12 mx-auto">
                        ...
                        <div
                            class="text-amber-200/90 font-medium uppercase text-sm md:text-base w-2/3 mx-auto tracking-wide">
                            Premier
                        </div>
                        <div class="text-pink-100 font-semibold text-sm md:text-base w-full mx-auto">
                    ...
                    <div class="cta_reserv pt-10 w-full mx-auto flex justify-center">
                        <router-link :to="`/movies/${film.movie.id}`"
                            class="bg-pink-600 hover:bg-pink-600/90 text-white text-lg tracking-wide font-bold py-4 px-8 rounded-full transition duration-300 shadow-md  shadow-pink-400/85">

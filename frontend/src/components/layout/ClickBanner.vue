<template>
  <div class="container mx-auto my-12 relative overflow-hidden">
    <div 
      class="carousel-container h-full min-w-full min-h-[70dvh]"
    ...
    >
      <div 
        ...
        :key="index"
        :class="['carousel-item absolute w-full h-full transition-transform duration-1000 ease-in-out', 
          { 'active-slide': index === activeIndex }
        ]"
        :style="getSlideTransform(index)"
      >
        <img :src="movie.imageUrl" :alt="movie.title" class="w-full h-full object-cover rounded-lg shadow-xl overflow-hidden">
        <div class="absolute  right-0 top-0 bg-gradient-to-b from-black to-transparent p-12 pb-24 text-white rounded-r-lg overflow-hidden">
          <h3 class="text-4xl font-bold mb-2">{{ movie.title }}</h3>
          <p class="text-xl">Nézd meg most a drive-in moziban!</p>
        </div>
      </div>
    </div>
    ...
    <div class="carousel-indicators absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 z-10">
      <button 
        ...
        :class="['w-4 h-4 rounded-full bg-gray-400 hover:bg-pink-500 transition-colors duration-300', { 'bg-pink-600': index === activeIndex }]"
      ></button>
    </div>
  </div>
</template>
...
const movies = ref([
  ...
  { imageUrl: 'https://sm.ign.com/ign_hu/screenshot/default/dune-2021-1200-1200-675-675-crop-000000_rqk4.jpg', title: 'Dűne' },
  { imageUrl: 'https://staticeu.sweet.tv/images/cache/v2/movie_banner/CPi6ARICZW4aAkhVIAIoFjAm/23928-star-wars-the-force-awakens_1280x720.jpg', title: 'Star Wars: Az ébredő Erő' },
]);
...
.carousel-item {
  ...
  left: 0; /* Alapértelmezett pozíció */
  top: 0;
}

...
<template>
  <BaseHeader />
  <div class="mx-auto md:mt-12 lg:mt-0 lg:w-full min-h-screen">
    ...
    <div class="container mx-auto w-full h-full mb-8">
      <div class="md:mt-12 text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-white leading-relaxed
        ...
        bg-clip-text text-transparent md:pt-1 lg:pt-0 pb-6 border-pink-500 w-full text-end">
          Csekkold a
          <router-link to="/movies/PremierWeek" class="hover:text-amber-300 transition-colors duration-200 ease-in-out">
            premier filmeket </router-link>
          <span class="text-rose-500/90 font-extrabold text-2xl lg:text-5xl">!</span>
        </p>
      </div>
      <div class="card mx-auto px-6 w-fit lg:w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 justify-center gap-6 md:gap-8 
        lg:gap-6 xl:gap-10">
        ...
        <router-link to="/movies/PremierWeek">
          <BaseCard :title="'Premier Filmek a Héten'" :type="'Premierek'"
            ...
            :img_size_class="'object-cover overflow-hidden'" :cta="true" :link="'/movies/PremierWeek'"
            :button_text="'Premier filmek'" />

        </router-link>
      </div>
    </div>
    <div class="md:my-16 lg:mb-32 lg:mt-24 mx-4">
      <MovieThemes />
    </div>
